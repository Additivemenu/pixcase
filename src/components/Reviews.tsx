...
function ReviewColumn({
  ...
  const duration = `${columnHeight * msPerPixel}ms`; // depending on the height of column, the duration of the marquee effect will be set (higher the column, faster the marquee effect)

  useEffect(() => {
    // mount an observer to watch for changes in the column height
...
function Review({ imgSrc, className, ...props }: ReviewProps) {
  ...
  return (
    <div
      // ! animate-fade-in is a custom animation class in tailwind.config.ts
      className={cn(
    ...
    >
      <Phone imgSrc={imgSrc} />
      {/* <div className="border-2 border-solid">123</div> */}
    </div>
  );
}
...
function ReviewGrid() {
  ...
  return (
    <div
      ref={containerRef}
      className="relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 items-start
...
const Reviews = () => {
  return (
    <MaxWidthWrapper className="relative max-w-5xl">
      ...
      />
      {/* with phone card flowing above animation effect */}
      <ReviewGrid />
    </MaxWidthWrapper>
  );
};
